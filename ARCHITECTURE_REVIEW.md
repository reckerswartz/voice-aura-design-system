...
1. **451 `@import` deprecation warnings** — All 29 `@import` statements in `voice-aura.scss` and `_bootstrap.scss` must migrate to `@use`/`@forward`. This is the single largest technical debt item. Dart Sass 3.0 will remove `@import` entirely.

2. **50+ deprecated `darken()`/`lighten()` calls** — Must migrate to `color.adjust()` or `color.scale()`. Affects `_feature-section.scss`, `_hero.scss`, `_navbar.scss`, `_pricing.scss`, `_backgrounds.scss`, `_animations.scss`.
...
4. **`_backgrounds.scss` is 1,545 lines** — This single file accounts for 14% of all SCSS. Should be split into logical sub-modules: patterns, glows, gradients, presets, section-blends.

5. **`_animations.scss` is 1,314 lines** — Second-largest file. Should be split: keyframes, scroll-triggers, hover-effects, responsive-morphs.

6. **Monolithic CSS output (340 KB min)** — No tree-shaking. Unused Bootstrap modules (modal, tooltip, dropdown, carousel, offcanvas) ship ~34 KB of dead CSS. A PostCSS pipeline with PurgeCSS would cut this significantly.

7. **No authored CSS custom properties** — All 54 `--va-*` properties are auto-generated by Bootstrap. Zero hand-authored custom properties means no runtime theming capability.

#### P2 — Medium (improves developer experience)

8. **No linting or CI** — No stylelint, no visual regression tests, no size budget enforcement. The `.github/` directory exists but has no workflows for CSS quality gates.
...
11. **Gradient token inconsistency** — Feature panel gradients use 145deg angle with `darken()`, while `$va-gradient-*` tokens use 135deg with `color.adjust()`. Should be unified (see `screenshots/audit/sections/GRADIENT_RADIUS_AUDIT.md`).
...
1. **Migrate `@import` → `@use`/`@forward`** — Eliminates 451 warnings, unblocks Sass 3.0
2. **Replace `darken()`/`lighten()` → `color.adjust()`** — Eliminates remaining deprecation warnings
3. **Split `_backgrounds.scss` and `_animations.scss`** — Improves maintainability
4. **Add PostCSS pipeline** (autoprefixer + cssnano + PurgeCSS) — Reduces CSS size by ~40%
5. **Add stylelint + size budget to CI** — Prevents regression
6. **Extract navbar into a reusable include** — Reduces HTML duplication
